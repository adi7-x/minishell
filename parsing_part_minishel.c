...
// 	new_file->infile = (current_token->type == TOKEN_INREDIR);
// 	new_file->outfile = (current_token->type == TOKEN_OUTREDIR);
// 	new_file->append = (current_token->type == TOKEN_REDIR_APPEND);
// 	new_file->heredoc = (current_token->type == TOKEN_HEREDOC);
...
// 						current_token->data);
// 			}
// 			else if (current_token->type == TOKEN_INREDIR
// 				|| current_token->type == TOKEN_OUTREDIR
...
// Function to search for an environment variable in env
...
//             return (env[i] + len + 1); // Return the value after the '=' sign
//         i++;
//     }
//     return (NULL);
// }

// ft_strjoin implementation
...
// 	// expand_variables(lexer_output, shell->env); 
	
	
//     data = ft_parser(lexer_output, shell);
//     if (!data)
//     {
